...
<body class="min-h-screen flex flex-col justify-between selection:bg-red-500 selection:text-white">
    ...
    <header class="w-full bg-white border-b border-slate-200 px-4 py-3 sm:px-8 flex items-center justify-between sticky top-0 z-40 shadow-sm">
        <div class="flex items-center gap-3">
            ...
            <div class="flex items-center gap-2">
                <img src="<?php echo $uploadPath; ?><?php echo htmlspecialchars($team['logo'] ? $team['logo'] : 'team_placeholder.jpg'); ?>" alt="Team Logo" class="w-7 h-7 sm:w-8 sm:h-8 object-contain bg-slate-50 p-0.5 rounded-lg border border-slate-200 shadow-sm">
                <div>
                    <h1 class="text-sm font-montserrat font-black uppercase tracking-tight text-slate-900 leading-none">
                        <?php echo htmlspecialchars($team['team_name']); ?>
                    </h1>
                    <p class="text-[9px] font-mono text-amber-700 font-bold uppercase tracking-wider mt-0.5">Manager Console</p>
                </div>
            </div>
        </div>
        ...
        <div class="flex items-center gap-2 sm:gap-3">
            ...
            <div class="flex items-center gap-1.5 bg-slate-100 border border-slate-200 rounded-xl px-3 py-1.5 text-xs shadow-sm">
                <span class="w-2 h-2 rounded-full bg-slate-400 animate-pulse" id="status-light"></span>
                <span class="text-slate-900 font-montserrat font-extrabold tracking-wider uppercase text-[10px]" id="status-text">Arena Idle</span>
            </div>
    ...
    <main class="flex-grow p-4 md:p-6 max-w-7xl w-full mx-auto space-y-6 relative">
        ...
        <div class="lg:col-span-4 space-y-6">
            <!-- Budget Purse Card -->
            <div class="pro-card rounded-2xl p-6 border-l-4 border-l-amber-500 relative overflow-hidden shadow-sm">
                <span class="text-[10px] text-slate-500 uppercase font-mono tracking-widest font-bold block">Remaining Purse Limit</span>
                <h2 class="text-4xl sm:text-5xl font-montserrat font-black text-amber-700 mt-1 tracking-tight" id="manager-purse">
                    ₹<?php echo number_format($team['remaining_purse']); ?>
                </h2>
                <div class="mt-4 flex items-center justify-between text-xs text-slate-600 border-t border-slate-100 pt-3 font-inter">
                    <span>Total Starting Purse:</span>
                    <span class="font-bold font-mono text-slate-900">₹<?php echo number_format($team['total_purse']); ?></span>
                </div>
            </div>

            <!-- Squad List Bento Card -->
            <div class="pro-card rounded-2xl p-6 flex flex-col h-[380px] shadow-sm">
                <div class="flex items-center justify-between border-b border-slate-100 pb-4 mb-4">
                    <div>
                        <span class="text-[10px] text-slate-500 uppercase font-mono tracking-widest font-bold">Squad List</span>
                        <h3 class="text-2xl font-montserrat font-black text-slate-900 mt-1" id="manager-squad-size">
                ...
                <div class="flex-grow overflow-y-auto space-y-2 pr-1" id="manager-squad-list">
                    ...
                    <div class="text-center text-xs text-slate-400 py-8 uppercase font-semibold">No players purchased yet.</div>
                </div>
                <p class="text-[10px] text-slate-500 font-mono mt-4 border-t border-slate-100 pt-3">Maximum roster is limited to <?php echo $team['max_squad_size']; ?> slots.</p>
            </div>
        </div>
        ...
        <div class="lg:col-span-8 grid grid-cols-1 md:grid-cols-12 gap-6" id="auction-console">
            <!-- Standby Box -->
            <div id="standby-box" class="col-span-12 pro-card rounded-2xl p-10 text-center flex flex-col items-center justify-center min-h-[380px] shadow-sm">
                <i class="fa-solid fa-tower-broadcast text-5xl text-amber-600 animate-pulse mb-4 block"></i>
                <h2 class="text-2xl font-montserrat font-black text-slate-900 uppercase tracking-tight">Waiting for Live Auctioneer</h2>
                <p class="text-slate-600 max-w-md mt-2 text-xs leading-relaxed font-inter">
            ...
            <div id="completed-stats-box" class="hidden col-span-12 pro-card rounded-2xl p-6 flex flex-col gap-6 relative overflow-hidden shadow-sm">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 relative z-10">
                    <div class="bg-slate-50 border border-slate-200 rounded-xl p-4 text-center shadow-sm">
                        ...
                        <span class="text-xl font-montserrat font-black text-amber-700 mt-1 block" id="stats-total-spent">₹0</span>
                    </div>
                    <div class="bg-slate-50 border border-slate-200 rounded-xl p-4 text-center shadow-sm">
                        <span class="text-[9px] text-slate-500 uppercase tracking-wider block font-bold">Avg. Player Bid</span>
                        <span class="text-xl font-montserrat font-black text-slate-900 mt-1 block" id="stats-avg-price">₹0</span>
                    </div>
                    <div class="bg-slate-50 border border-slate-200 rounded-xl p-4 text-center shadow-sm">
                        <span class="text-[9px] text-slate-500 uppercase tracking-wider block font-bold">Players Sold</span>
                        <span class="text-xl font-montserrat font-black text-emerald-600 mt-1 block" id="stats-sold-count">0</span>
                    </div>
                    <div class="bg-slate-50 border border-slate-200 rounded-xl p-4 text-center shadow-sm">
                        <span class="text-[9px] text-slate-500 uppercase tracking-wider block font-bold">Unsold Players</span>
                        <span class="text-xl font-montserrat font-black text-red-600 mt-1 block" id="stats-unsold-count">0</span>
                    </div>
                </div>
            ...
            <div id="player-card" class="hidden col-span-12 md:col-span-4 pro-card p-5 rounded-2xl flex flex-col justify-between relative shadow-sm">
                <!-- Top Header: Country/Location & Role -->
                <div class="flex items-center justify-between relative z-10 mb-3">
                    <div class="flex items-center gap-1.5 bg-slate-100 border border-slate-200 px-2.5 py-1 rounded-lg">
                        <span class="text-xs">🇮🇳</span>
                        <span class="text-[9px] font-mono font-bold text-slate-800 uppercase tracking-wider" id="player-place">
                            Wayanad
                        </span>
                    </div>
                    <span class="bg-red-100 border border-red-200 text-red-700 text-[8px] font-mono font-bold px-2.5 py-1 rounded-lg uppercase tracking-wider" id="player-role">
                        ROLE
                    </span>
                </div>
                ...
                <div class="relative z-10 mb-2 text-center">
                    <h2 class="text-xl font-montserrat font-black text-slate-900 uppercase tracking-tight leading-none" id="player-name">Player Name</h2>
                </div>

                <!-- Center Stage: Cutout Player Image -->
                <div class="relative z-10 my-3 flex justify-center">
                    <div class="w-32 h-36 rounded-2xl overflow-hidden border-2 border-red-500 bg-slate-100 shadow-md relative cursor-pointer" onclick="openImageLightbox(document.getElementById('player-image').src, document.getElementById('player-name').innerText);">
                ...
                <div class="relative z-10 bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 flex justify-between items-center text-xs">
                    <span class="text-slate-500 font-mono font-bold uppercase tracking-wider text-[10px]">Base Price</span>
                    <span class="text-blue-700 font-mono font-black text-sm" id="player-base-price">₹100</span>
                </div>
            </div>
            ...
            <div id="bid-action-card" class="hidden col-span-12 md:col-span-5 space-y-6 flex flex-col justify-between">
                ...
                <!-- Bidding Panel -->
                <div class="pro-card rounded-2xl p-5 border border-slate-200 flex-grow flex flex-col justify-between shadow-sm">
                    <div>
                        <div class="flex justify-between items-center">
                            <span class="text-[10px] text-slate-500 uppercase font-mono tracking-widest font-bold">Current Active Bid</span>
                            <span class="text-[8px] font-mono uppercase tracking-widest font-bold text-white bg-red-600 border border-red-600 px-2 py-0.5 rounded shadow animate-pulse" id="high-bidder-indicator" style="display:none;">Leading</span>
                        </div>
                        <h3 class="text-4xl font-montserrat font-black text-red-600 mt-1 tracking-tight" id="active-bid">₹0</h3>
                        <div class="flex items-center gap-2 mt-1.5" id="leading-team-wrapper">
                            <div id="leading-team-logo-container" class="w-7 h-7 rounded bg-slate-100 border border-slate-200 flex items-center justify-center overflow-hidden p-0.5" style="display: none;">
                                <img src="" id="leading-team-logo" class="w-full h-full object-contain">
                            </div>
                            <p class="text-xs text-slate-500 font-inter" id="leading-team">No bids placed yet</p>
                        </div>
                    </div>

                    <!-- Bid Increments Buttons -->
                    <div class="mt-6">
                        <span class="block text-[9px] text-slate-500 font-mono uppercase tracking-widest font-bold mb-3">Quick Incremental Bids</span>
                    ...
                    <div class="mt-6 border-t border-slate-100 pt-4">
                        <span class="block text-[9px] text-slate-500 font-mono uppercase tracking-widest font-bold mb-2">Place Custom Bid (₹)</span>
                        <div class="flex gap-2">
                            <input type="number" id="custom-bid-input" placeholder="Enter bid amount" min="100" step="100"
                                   class="flex-grow bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-xs text-slate-900 placeholder-slate-400 focus:outline-none focus:border-red-600 transition font-mono font-bold">
                            <button id="custom-bid-submit" onclick="submitCustomBid()"
                                   class="bg-red-600 hover:bg-red-700 text-white font-montserrat font-extrabold uppercase text-xs tracking-wider px-5 rounded-xl transition shadow-sm">
                                Bid
                            </button>
                        </div>
                    </div>
            ...
            <div id="bid-history-card" class="hidden col-span-12 md:col-span-3 pro-card rounded-2xl p-4 border border-slate-200 flex flex-col h-full overflow-y-auto shadow-sm">
                <span class="block text-[9px] text-slate-500 font-mono uppercase tracking-widest font-bold mb-2.5 border-b border-slate-100 pb-1.5 flex items-center gap-1.5">
                    <i class="fa-solid fa-clock-rotate-left text-amber-600"></i> Live Bid Stream
                </span>
    ...
    <div class="w-full">
            ...
            <div class="pro-card rounded-2xl p-6 shadow-sm flex flex-col min-h-[350px]">
                <div class="border-b border-slate-100 pb-4 mb-4 space-y-3">
                    <div class="flex items-center justify-between gap-3">
                        <div>
                            <span class="text-[10px] text-slate-500 font-mono uppercase tracking-widest font-bold">Player Auction Pool</span>
                            <h3 class="text-base font-montserrat font-black text-slate-900 flex items-center gap-1.5 uppercase">
                                <i class="fa-solid fa-circle-check text-emerald-600"></i> Player Auctions Status
                            </h3>
                        </div>
                        <div class="relative w-40 sm:w-52">
                            ...
                            <input type="text" id="completed-search-input" onkeyup="filterCompletedPlayers()" placeholder="Search status..." 
                                   class="w-full bg-slate-50 border border-slate-200 rounded-xl pl-8 pr-3 py-1.5 text-xs text-slate-900 placeholder-slate-400 focus:outline-none focus:border-red-600 transition font-inter">
                        </div>
                    </div>
                    <!-- Filter Chips -->
                    <div class="flex items-center gap-2 overflow-x-auto pb-1 shrink-0 scrollbar-none" id="manager-status-filter-container">
                        <button onclick="setManagerStatusFilter('all')" class="status-chip px-3 py-1.5 rounded-lg border text-[10px] font-montserrat uppercase font-extrabold tracking-wider transition bg-red-600 border-red-600 text-white" data-filter="all">ALL</button>
                        <button onclick="setManagerStatusFilter('Sold')" class="status-chip px-3 py-1.5 rounded-lg border text-[10px] font-montserrat uppercase font-extrabold tracking-wider transition border-slate-200 bg-slate-100 text-slate-700 hover:border-red-600 hover:text-slate-900" data-filter="Sold">SOLD</button>
                        <button onclick="setManagerStatusFilter('Unsold')" class="status-chip px-3 py-1.5 rounded-lg border text-[10px] font-montserrat uppercase font-extrabold tracking-wider transition border-slate-200 bg-slate-100 text-slate-700 hover:border-red-600 hover:text-slate-900" data-filter="Unsold">UNSOLD</button>
                        <button onclick="setManagerStatusFilter('Available')" class="status-chip px-3 py-1.5 rounded-lg border text-[10px] font-montserrat uppercase font-extrabold tracking-wider transition border-slate-200 bg-slate-100 text-slate-700 hover:border-red-600 hover:text-slate-900" data-filter="Available">AVAILABLE</button>
                    </div>
                </div>
                <div class="flex-grow overflow-y-auto max-h-[300px] space-y-2.5 pr-1" id="completed-pool-container">
                    <?php if (empty($completedPool)): ?>
                        <div class="text-center text-slate-500 text-xs py-10 uppercase tracking-widest font-semibold no-players-msg font-inter">
                    ...
                    <?php else: ?>
                        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                            <?php foreach ($completedPool as $cp): 
                                ...
                                if ($cp['auction_status'] === 'Sold') $statusBadgeClass = 'bg-emerald-600 text-white';
                                elseif ($cp['auction_status'] === 'Unsold') $statusBadgeClass = 'bg-red-600 text-white';
                                elseif ($cp['auction_status'] === 'Available') $statusBadgeClass = 'bg-amber-500 text-slate-950';
                            ?>
                                <div class="completed-player-card bg-slate-50 border border-slate-200 rounded-xl p-3 flex items-center gap-3 shadow-sm hover:border-slate-300 transition"
                                     data-name="<?php echo htmlspecialchars(strtolower($cp['name'])); ?>"
                                     data-role="<?php echo htmlspecialchars(strtolower($cp['role'])); ?>"
                                     data-place="<?php echo htmlspecialchars(strtolower($cp['place'])); ?>"
                                    ...
                                    <div class="flex-grow min-w-0">
                                        <div class="flex items-center justify-between gap-1 mb-0.5">
                                            <span class="text-xs font-montserrat font-bold text-slate-900 truncate"><?php echo htmlspecialchars($cp['name']); ?></span>
    ...
    <!-- Footer Section -->
    <footer class="bg-white border-t border-slate-200 py-6 px-4 text-center text-xs text-slate-500 font-inter mt-8">
        <div class="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <div class="flex gap-4">
                <a href="../public/landing.php" class="hover:text-slate-900 transition">Home</a>
                <a href="../public/today_auctions.php" class="hover:text-slate-900 transition">Live Arena</a>
                <a href="https://wa.me/917698767767" target="_blank" class="hover:text-emerald-600 transition">Support</a>
            </div>
        </div>
    </footer>
        ...
        function setManagerStatusFilter(filter) {
            ...
            document.querySelectorAll('#manager-status-filter-container .status-chip').forEach(btn => {
                if (btn.getAttribute('data-filter') === filter) {
                    btn.className = 'status-chip px-3 py-1.5 rounded-lg border text-[10px] font-montserrat uppercase font-extrabold tracking-wider transition bg-red-600 border-red-600 text-white';
                } else {
                    btn.className = 'status-chip px-3 py-1.5 rounded-lg border text-[10px] font-montserrat uppercase font-extrabold tracking-wider transition border-slate-200 bg-slate-100 text-slate-700 hover:border-red-600 hover:text-slate-900';
                }
            });
